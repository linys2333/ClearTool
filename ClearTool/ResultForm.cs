...
namespace ClearTool
{
    public partial class ResultForm : PanelForm
    {
        ...
        private void dgvResult_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            if (cb != null && dgvResult.Columns[dgvResult.CurrentCell.ColumnIndex].Name == "IsClear")
            {
                cb.SelectedIndexChanged += clear_SelectedIndexChanged;
            }
        }

        private void clear_SelectedIndexChanged(object sender, EventArgs e)
        {
            ComboBox cb = sender as ComboBox;
            if (cb != null )
            {
                // 不撤销事件，会导致selectedchanged一直触发
                cb.Leave += clear_Leave;

                var buCell = dgvResult.CurrentCell.OwningRow.Cells[dgvResult.CurrentCell.ColumnIndex + 1];

                buCell.Value = cb.Text;
                buCell.ReadOnly = cb.Text == "否";
            }
        }

        private void clear_Leave(object sender, EventArgs e)
        {
            ComboBox cb = sender as ComboBox;
            if (cb != null)
            {
        ...
        private void ReverseBind()
        {
            for (int i = 0; i < dgvResult.Rows.Count; i++)
            {
                ...
                var isClearOfBU = Util.ConvertType<string>(row.Cells["IsClearOfBU"].Value);

                _comparaResult.SingleOrNew(e => e.TableName == tableName).IsClear = isClear == "是";
                _comparaResult.SingleOrNew(e => e.TableName == tableName).IsClearOfBU = isClearOfBU == "是";
            }
        }
